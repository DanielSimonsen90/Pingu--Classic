...
const { PinguLibrary, PinguGuild, PinguUser, DiscordPermissions, PClient, TimeLeftObject } = require('PinguPackage');
const ms = require('ms');

module.exports = {
    name: 'slowmode',
    description: 'Sets the slowmode to specified argument in channel',
    usage: '[#channel] <slowmode in seconds>',
    guildOnly: true,
    id: 1,
    examples: [""],
    permissions: [DiscordPermissions.SEND_MESSAGES, DiscordPermissions.MANAGE_CHANNELS],
    aliases: [""],
...
function PermissionCheck(message, args) {
    ...
    return PinguLibrary.PermissionCheck(message, [DiscordPermissions.MANAGE_CHANNELS]);
}
/**@param {Message} message
 * @param {string[]} args
 * @returns {TextChannel}*/
