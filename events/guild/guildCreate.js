...
module.exports = new PinguEvent('guildCreate', 
    async function setContent(client, embed, guild) {
        return module.exports.content = embed.setDescription(joinString(guild.client, guild));
    },
    async function execute(client, guild) {
        ...
        let ownerDM = await owner.user.createDM();
        if (!ownerDM) return client.log('error', `Unable to create DM to ${owner}!`, null, null, { params: { guild } });
        ...
        try {
            ...
            client.log('console', `Sent ${owner.user.tag} my "thank you" message.`)
        } catch (err) {
            client.log('error', `Failed to send ${owner} a DM`, null, err, { params: { guild }, additional: { OwnerDM: ownerDM } })
        }
    }
);