        ...
        try {
            try {
                if (event.execute != null)
                    event.execute(PinguClient_1.ToPinguClient(client), ...args);
            }
            catch (err) {
            ...
            PinguLibrary.errorLog(client, `Cause: ${cause}`, JSON.stringify(args, null, 2), null, PinguLibrary.errorCache.size - 1);
        }
        function SendToLog() {
            return __awaiter(this, void 0, void 0, function* () {
                var parameters = Object.assign({}, ...args);
                let emitAssociator = parameters.author && parameters.author.tag ||
                    parameters.tag ||
                    parameters.user && parameters.user.tag ||
                    parameters.member && parameters.member.user.tag ||
                    parameters.users && parameters.users.cache.last() && parameters.users.cache.last().tag ||
                    parameters.last && parameters.last() && parameters.last().author.tag ||
                    parameters.inviter && parameters.inviter.tag ||
                    parameters.name ||
                    parameters.guild && parameters.guild.name ||
                    "Unknown";
                let specialEvents = [
                ...
                if (embed)
                    return yield PinguLibrary.eventLog(PinguClient_1.ToPinguClient(client), embed);
                function GetFromAuditLog() {
                    return __awaiter(this, void 0, void 0, function* () {
                        const noAuditLog = PinguEvent.noAuditLog;
                        switch (event.name) {
                            case 'channelCreate': return !parameters.channel.guild ? parameters.channel.recipient.tag : yield GetInfo(parameters.channel.guild, 'CHANNEL_CREATE');
                            case 'channelUpdate': return !parameters.channel.guild ? parameters.channel.recipient.tag : yield GetInfo(parameters.channel.guild, 'CHANNEL_UPDATE');
                            case 'channelDelete': return !parameters.channel.guild ? parameters.channel.recipient.tag : yield GetInfo(parameters.channel.guild, 'CHANNEL_DELETE');
                            case 'channelPinsUpdate': return !parameters.channel.guild ? parameters.channel.recipient.tag :
                                ((yield GetInfo(parameters.channel.guild, 'MESSAGE_PIN')) ||
                                    (yield GetInfo(parameters.channel.guild, 'MESSAGE_UNPIN')));
                            case 'webhookCreate': return yield GetInfo(parameters.channel.guild, 'WEBHOOK_CREATE');
                            case 'webhookUpdate': return yield GetInfo(parameters.channel.guild, 'WEBHOOK_UPDATE');
                            case 'webhookDelete': return yield GetInfo(parameters.channel.guild, 'WEBHOOK_DELETE');
                            case 'emojiCreate': return yield GetInfo(parameters.emote.guild, 'EMOJI_CREATE');
                            case 'emojiUpdate': return yield GetInfo(parameters.emote.guild, 'EMOJI_UPDATE');
                            case 'emojiDelete': return yield GetInfo(parameters.emote.guild, 'EMOJI_DELETE');
                            case 'guildBanAdd': return yield GetInfo(parameters.guild, 'MEMBER_BAN_ADD');
                            case 'guildMemberUpdate': return yield GetInfo(parameters.member.guild, 'MEMBER_UPDATE');
                            case 'guildBanRemove': return yield GetInfo(parameters.guild, 'MEMBER_BAN_REMOVE');
                            case 'guildUpdate': return yield GetInfo(parameters.guild, 'GUILD_UPDATE');
                            case 'guildIntegrationsUpdate': return yield GetInfo(parameters.guild, 'INTEGRATION_UPDATE');
                            case 'messageBulkDelete': return yield GetInfo(parameters.messages.last().guild, 'MESSAGE_BULK_DELETE');
                            case 'roleCreate': return yield GetInfo(parameters.role.guild, 'ROLE_CREATE');
                            case 'roleUpdate': return yield GetInfo(parameters.role.guild, 'ROLE_UPDATE');
                            case 'roleDelete': return yield GetInfo(parameters.role.guild, 'ROLE_DELETE');
                            default:
