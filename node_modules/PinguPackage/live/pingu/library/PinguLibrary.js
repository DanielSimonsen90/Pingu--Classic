...
exports.PinguLibrary = exports.RequestImage = exports.DBExecute = exports.getEmote = exports.BlankEmbedField = exports.getImage = exports.raspberryLog = exports.latencyCheck = exports.tellLog = exports.eventLog = exports.consoleLog = exports.pUserLog = exports.pGuildLog = exports.errorLog = exports.errorCache = exports.DanhoDM = exports.outages = exports.getTextChannel = exports.isPinguDev = exports.CacheDevelopers = exports.Developers = exports.getSharedServers = exports.getServer = exports.SavedServers = exports.PinguSupportInvite = exports.Permissions = exports.PermissionCheck = exports.PermissionGranted = void 0;
...
function errorLog(client, message, messageContent, err, errorID) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!errorlogChannel)
            return PinguLibrary.DanhoDM(`Unable to find #error-log-⚠️ in Pingu Support, ${PinguLibrary.PinguSupportInvite}`);
        //Console error
        ...
        if (!PinguLibrary.errorCache.get(errorID) && err)
            fs.writeFile(errorFilePath, JSON.stringify(err), () => consoleLog(client, `Created error file for error #${errorID}.`));
        else
            return fs.appendFile(errorFilePath, messageContent, () => consoleLog(client, `Appended ${message} to error #${errorID}`));
        //Send and react
        ...
        if (!PinguLibrary.errorCache.get(errorID)) {
            yield sent.react(PinguLibrary.SavedServers.DanhoMisc(client).emojis.cache.find(e => e.name == 'Checkmark')); //Mark as resolved
        ...
        sent.createReactionCollector((reaction, user) => ['Checkmark', '📄'].includes(reaction.emoji.name) &&
            ...
            reaction.message.guild.id == PinguLibrary.SavedServers.PinguSupport(client).id).on('collect', (reaction, user) => {
            if (!PinguLibrary.isPinguDev(user))
...
function eventLog(client, content) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!eventLogChannel)
            return PinguLibrary.DanhoDM(`Couldn't get #event-log-📹 channel in Pingu Support, ${PinguLibrary.PinguSupportInvite}`);
...
function tellLog(client, sender, reciever, message) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!tellLogChannel)
            return PinguLibrary.DanhoDM(`Couldn't get #tell-log-💬 channel in Pingu Support, ${PinguLibrary.PinguSupportInvite}`);
        if (message.constructor.name == "Message") {
            var messageAsMessage = message;
            var consoleLogValue = messageAsMessage.content ?
            ...
            if (messageAsMessage.content && messageAsMessage.attachments)
                ...
                errorLog(client, `${sender} ➡️ ${reciever} sent something that didn't have content or attachments`)
...
function latencyCheck(message) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!pingChannel)
            return PinguLibrary.DanhoDM(`Couldn't get #ping-log-🏓 channel in Pingu Support, ${PinguLibrary.PinguSupportInvite}`);
...
function raspberryLog(client) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!raspberryLogChannel)
            return PinguLibrary.DanhoDM(`Couldn't get #raspberry-log-🍇 channel in Pingu Support, ${PinguLibrary.PinguSupportInvite}`);
...
function RequestImage(message, pGuildClient, caller, types, searchTerm) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!config || !config.api_key || !config.google_custom_search) {
            return PinguLibrary.errorLog(client, `Unable to send ${caller}\nImage search requires both a YouTube API key and a Google Custom Search key!`).then(() => message.channel.send(`I was unable to search for a ${type}! I have contacted my developers...`));
        }
        ...
        request('https://www.googleapis.com/customsearch/v1?key=' + config.api_key + '&cx=' + config.google_custom_search + '&q=' + (searchTerm(type)) + '&searchType=image&alt=json&num=10&start=' + page, (err, res, body) => __awaiter(this, void 0, void 0, function* () {
            if (err)
                return PinguLibrary.errorLog(message.client, `Error getting results when searching for ${searchTerm(type)}`, message.content, new helpers_1.Error(err));
            ...
            try {
                ...
                PinguLibrary.errorLog(message.client, `Getting data in ${caller}, PinguLibrary.RequestImage()`, message.content, new helpers_1.Error(err));
            }
            if (!data) {
                return PinguLibrary.errorLog(message.client, `Getting data in ${caller}, PinguLibrary.RequestImage()\n\n${JSON.stringify(data)}`).then(() => message.channel.send(`I was unable to recieve a gif! I have contacted my developers...`));
            }
            else if (!data.items || data.items.length == 0) {
                return PinguLibrary.errorLog(message.client, `Data for ${caller} has no items\n\n${data}`).then(() => message.channel.send(`I was unable to find a gif! I have contacted my developers...`));
            }
...
class PinguLibrary {
    ...
    static errorLog(client, message, messageContent, err, errorID) {
        return __awaiter(this, void 0, void 0, function* () { return errorLog(client, message, messageContent, err, errorID); });
    }
    ...
    //#endregion
    //#region Statics
    static getEmote(client, name, emoteGuild) { getEmote(client, name, emoteGuild); }
    static getImage(script, imageName) { return getImage(script, imageName); }
    static DBExecute(client, callback) {
        return __awaiter(this, void 0, void 0, function* () { return DBExecute(client, callback); });
    }
    static BlankEmbedField(inline = false) { return BlankEmbedField(inline); }
