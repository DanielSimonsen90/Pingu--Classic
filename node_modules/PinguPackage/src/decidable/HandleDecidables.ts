...
import { PClient, PGuildMember, PRole, PChannel } from '../database'; //PItems
...
async function FirstTimeExecuted(params: DecidablesParams) {
    ...
    let staffRoleType = 
        decidablesType == DecidablesTypes.Giveaway ? 'Giveaway Host' : 
        decidablesType == DecidablesTypes.Poll ? 'Poll Host' : 
        decidablesType == DecidablesTypes.Suggestion ? 'Suggestion Manager' :
        'Theme Host';
    ...
    collector.on('collect', async input => {
        ...
        switch (collectorCount) {
            ...
            case 3:
                ...
                if (![DecidablesTypes.Giveaway, DecidablesTypes.Theme].includes(decidablesType)) return GoodToGo({ staffRole, decidablesChannel });
    ...
    async function GoodToGo(params: { staffRole: RoleResolvable, decidablesChannel: GuildChannelResolvable, winnerRole?: RoleResolvable, allowSameWinner?: boolean, ignoreLastWins?: number }) {
        ...
        return decidablesType == DecidablesTypes.Giveaway ? pGuild.giveawayConfig :
            decidablesType == DecidablesTypes.Poll ? pGuild.pollConfig : 
            decidablesType == DecidablesTypes.Suggestion ? pGuild.suggestionConfig :
            pGuild.themeConfig;
