...
import { errorLog, PermissionGranted, AchievementCheck, consoleLog, PermissionCheck, getEmote, SavedServers } from '../pingu/library/PinguLibrary';

import TimeLeftObject from '../helpers/TimeLeftObject';

import ms from 'ms';

type DecidablesTypes = 'Giveaway' | 'Poll' | 'Suggestion' | 'Theme';
...
export async function HandleDecidables(params: DecidablesParams) {
    ...
    if (!pGuild) {
        await errorLog(client, `Unable to host ${decidablesType.toLowerCase()} for ${message.guild.name}, as I couldn't get their PinguGuild!`);
    ...
    if (permCheck != PermissionGranted) return send(permCheck);
    ...
    for (const [u, perms] of users) {
        let channelPerms = PermissionCheck({ author: u, client, content, channel: decidablesChannel }, ...perms);
        if (channelPerms != PermissionGranted) return send(channelPerms);
    }
    ...
    if ('Suggestion' != decidablesType) var endsAt = new Date(Date.now() + ms(time));
    ...
    if (reroll()) {
        let sent = await send(`Rerolling ${decidablesType.toLowerCase()}...`);
        return Reroll(params, sent, embed, Configs.get(config).collection.find(d => d._id == value) as any)
    }
    else if (message.channel.id == decidablesChannel.id &&
        PermissionCheck(
            { author: client.user, client, channel: decidablesChannel },
            'MANAGE_MESSAGES'
        ) == PermissionGranted)
        message.delete();
    else send(`Announcing the ${decidablesType.toLowerCase()} in ${decidablesChannel} now!`).then(s => s.delete({ timeout: ms('5s') }));

    const sent = await send(`New ${decidablesType}!`, embed);
    reactionEmojis.forEach(e => sent.react(e));
    consoleLog(client, `${author} hosted ${decidablesType} in ${decidablesChannel} (${decidablesChannel.name}), ${guild}`);

    let decidable = new Decidable(value, sent.id, new PGuildMember(member), decidablesChannel, endsAt);
    ...
    let interval = setInterval(() => function updateTimer() {
        sent.edit(sent.embeds[0].setDescription(
            ...
            errorLog(client, `Updating ${decidablesType.toLowerCase()} timer`, content, err);
    ...
    setTimeout(() => onTimeFinished(sent, value, winners, embed, decidable, interval, params, null), ms(time));
}
...
async function PermissionCheckDecidable(params: DecidablesParams) {
    ...
    else if (!isNaN(arg0Parsed)) args[0] += "m"; //No s/m/h provided, treat as minutes
...
async function FirstTimeExecuted(params: DecidablesParams) {
    ...
    collector.on('collect', async function* onCollect(input) {
        ...
        //hostRole Tag
        yield send(Find('Role', staffRoleType, 'Tag', userInput));
        ...
        if (['Giveaway', 'Theme'].includes(decidablesType)) {
            yield send(Find('Role', `${decidablesType} Winner`, 'Exists', userInput));
        ...
        //channel Tag
        yield send(Find('Channel', decidablesChannelName, 'Exists', userInput));
        ...
        //allowSameWinner Ask
        yield send(`Alright last thing, should I allow same winners? (A user winning a ${decidablesType.toLowerCase()} can't win the next one, if you say no!)`);
    ...
    collector.on('end', async () => {
        ...
        consoleLog(client, `"${message.guild.name}" successfully configured their ${decidablesType}Config.`);
    ...
    async function GoodToGo(params: { staffRole: RoleResolvable, channel: GuildChannelResolvable, winnerRole?: RoleResolvable, allowSameWinner?: boolean, ignoreLastWins?: number }) {
        ...
        async function MakeRole(guild: Guild, name: string): Promise<Role> {
            return guild.roles.create({
                ...
                errorLog(client, `Creating ${name} role`, message.content, err)
                return null;
            });
        }
        async function MakeChannel(guild: Guild, name: string): Promise<TextChannel> {
            return guild.channels.create(name, { 
                ...
                errorLog(client, `Creating ${name} channel`, message.content, err)
                return null;
            });
        }
    }
    ...
    function Find(type: 'Role' | 'Channel', typeName: string, pinguResponse: 'Exists' | 'Tag' | 'Find', userInput: Message): string {
        ...
        switch (pinguResponse) {
            ...
            case 'Find':
                ...
                if (typeResult) send(`Okay, I found ${typeResult.name}`);
...
async function onTimeFinished(sent: Message, value: string, winnersAllowed: number, embed: MessageEmbed, decidable: Decidable, interval: NodeJS.Timeout, params: DecidablesParams, previousWinners: Array<GuildMember>) {
    ...
    consoleLog(client, `Updated ${decidablesType.toLowerCase()} "${decidable._id}" after timeout.`);
    ...
    async function DecidePoll(poll: Poll) {
        ...
        consoleLog(client, `Poll, "${poll.value}" (${poll._id}) by ${poll.author.name}, voted ${poll.approved}.`);
    ...
    async function GetGiveawayWinner(decidable: Giveaway | Theme) {
        ...
        let reactedUsers = await sent.reactions.cache.get(reactionEmojis[0]).users.fetch({
            after: sent.createdAt.toString(),
            before: new Date().toString()
        }).catch(async err => {
            errorLog(client, `Fetching ${reactionEmojis[0]} reaction from ${decidablesType.toLowerCase()}`, sent.content, err);
        ...
        reactedUsers = reactedUsers.filter(u => {
            ...
            const userIsntInPreviousWinners = (themeConfig.collection as Theme[])
        ...
        let announceMessage = await sent.channel.send(`The ${winnerOrWinners.toLowerCase()} of **${value}** is no other than ${winnersString}! Congratulations!`);
        announceMessage.react(getEmote(client, 'hypers', SavedServers.get('Pingu Support')));
        ...
        for (let i = 0; i < winners.length; i++) {
            await guild.member(winners[i]).roles.add(winnerRole._id)
                .catch(err => {
                    errorLog(client, `Unable to give ${winners[i]} a ${decidablesType} "${guild}"'s ${decidablesType} Winner role, ${winnerRole.name} (${winnerRole._id})`, sent.content, err);
        ...
        sent.edit(embed
            ...
            .setDescription(
                `**${winnerOrWinners}:** ${winnersString}\n` +
                `**Host:** ${host}\n` +
                `**ID:** ${decidable._id}`
            )
            .setFooter(`${decidablesType} ended.`)
        ).catch(err => {
            errorLog(client, `Editing the ${decidablesType} Message`, sent.content, err);
...
async function ListDecidables(params: DecidablesParams, collection: Decidable[]) {
    ...
    if (!decidablesType.length || !embeds.length) return send(`There are no ${decidablesType.toLowerCase()}s saved!`);

    var sent = await send(embeds[embedIndex]);
    ...
    collector.on('end', async collected => {
        if (!collected.map(r => r.emoji.name).includes('🛑')) {
            await sent.delete();
            send(`Stopped showing ${decidablesType.toLowerCase()}s.`).then(s => s.delete({ timeout: ms('5s') }));
        }
    });
    collector.on('collect', async reaction => {
        ...
        async function onDefault() {
            errorLog(client, `${decidablesType.toLowerCase()}, ListDecidables(), collector.on(), default case: ${reaction.emoji.name}`, message.content);
    ...
    function CreateEmbeds(autoCalled: boolean) {
        ...
        for (let i = 0; i < collection.length; i++) {
            try {
                ...
                errorLog(client, `Error while adding ${decidablesType.toLowerCase()} to embeds`, message.content, err);
...
async function RemoveDecidables(message: Message, pGuild: PinguGuild, type: DecidablesTypes, decidables: Decidable[]) {
    ...
    if (logs.length) consoleLog(client, logs.join('\n'));
...
async function Reroll(params: DecidablesParams, sent: Message, embed: MessageEmbed, decidable: Giveaway | Theme) {
    const { message, args, decidablesType, client } = params;
    const { guild, channel, author } = message;
    const { edit } = sent;

    if (!args[1]) return edit(`${decidablesType.toLowerCase()} message not found - please provide a message ID.`);

    let previousMessage = channel.messages.cache.get(args[1]);
    if (previousMessage) {
        previousMessage = channel.messages.cache.get(args[1].split('/')[6]);
        if (previousMessage) return edit(`Unable to parse ${args[1]} as ID, or message can't be found!`);
        else if (!previousMessage.embeds[0]) return edit(`There's no embed in that message!`);
        else if (previousMessage.author.id != client.id) return edit(`That isn't my message!`);
    }

    const previousWinners = new Array<GuildMember>();
    for (const winner of decidable.winners) {
        previousWinners.push(await guild.members.fetch(winner._id));
    }
...
async function UpdatePGuild(client: PinguClient, pGuild: PinguGuild, decidableType: DecidablesTypes, reason: string) {
    return PinguGuild.Update(client, ['settings'], pGuild, `HandleDecidables: ${decidableType}`, reason);
}
