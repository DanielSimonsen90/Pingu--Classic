...
export async function HandleDecidables(params: DecidablesParams) {
    ...
    else decidablesChannel = (args[0] != 'reroll' ? guild.channels.cache.get(channel?._id) || message.channel : message.channel) as TextChannel;
    ...
    if (reroll) {
        ...
        return Reroll(params, sent, Configs.get(config).collection.find(d => d._id == value.split(' ')[1]) as any)
    }
        ...
        message.delete();
    else message.channel.send(`Announcing the ${decidablesType.toLowerCase()} in ${decidablesChannel} now!`).then(s => s.delete({ timeout: ms('5s') }));

    const sent = await message.channel.send(embed);
    reactionEmojis.forEach(e => sent.react(e));
    sent.createReactionCollector(
        (r: MessageReaction) => reactionEmojis.includes(r.emoji.name), 
        { time: endsAt.getTime() }
    ...
    let decidable = new (Configs.get(config).constructor)(value, sent.id, new PGuildMember(member), decidablesChannel, endsAt);
...
async function PermissionCheckDecidable(params: DecidablesParams) {
    ...
    else if (arg0.endsWith('s') && arg0Parsed < 30) return `Please specify a time higher than 29s!`;
    else if (!ms(arg0)) return `Please provide a valid time!`;
    else if (arg0.length == arg0Parsed.toString().length) args[0] += "m"; //No s/m/h provided, treat as minutes
...
async function FirstTimeExecuted(params: DecidablesParams) {
    ...
    const collector = message.channel.createMessageCollector(m => (m as Message).author.id == message.author.id);
    ...
    async function GetGiveawayWinner(decidable: Giveaway | Theme) {
        ...
        if (winnerRole) {
            ...
            for (let i = 0; i < winners.length; i++) {
                await guild.member(winners[i]).roles.add(winnerRole._id)
                .catch(err => {
        ...
        function getWinner() {
            let winner = (function selectWinner() {
                ...
                else if (guild.member(winner).roles.cache.has(winnerRole._id)) return allowSameWinner ? winner : null;
                return winner;
            })();
        ...
        async function UpdatePGuildWinners() {
            for (const winner of winners) {
                if (typeof winner != 'string') 
                    decidable.winners.push(new PGuildMember(guild.member(winner)));
            }
        }
    }
}
...
async function ListDecidables(params: DecidablesParams, collection: Decidable[]) {
    ...
    const collector = sent.createReactionCollector(
        (r: MessageReaction, u: User) => listEmojis.includes(r.emoji.name) && u.id == message.author.id, 
        { time: ms('20s') }
    );
...
async function Reroll(params: DecidablesParams, sent: Message, decidable: Giveaway | Theme) {
