...
export async function HandleDecidables(params: DecidablesParams) {
    ...
    const decidablesConfig = pGuild.settings.config;
...
async function PermissionCheckDecidable(params: DecidablesParams) {
    ...
    async function CheckRoleUpdates() {
        ...
        let winnerPRole = DecidablesEnum.Giveaway ? pGuild.settings.config.giveawayConfig.winnerRole : 
            DecidablesEnum.Theme ? pGuild.settings.config.themeConfig.winnerRole : undefined;
...
async function FirstTimeExecuted(params: DecidablesParams) {
    ...
    async function GoodToGo(params: { staffRole: RoleResolvable, decidablesChannel: GuildChannelResolvable, winnerRole?: RoleResolvable, allowSameWinner?: boolean, ignoreLastWins?: number }) {
        ...
        switch (decidablesType) {
            case DecidablesEnum.Giveaway: pGuild.settings.config.giveawayConfig = new GiveawayConfig({
                firstTimeExecuted, allowSameWinner, channel, hostRole: managerRole,
                winnerRole: finalWinnerRole, giveaways: []
            }); break;
            case DecidablesEnum.Poll: pGuild.settings.config.pollConfig = new PollConfig({
                firstTimeExecuted, channel, pollRole: managerRole, polls: []
            }); break;
            case DecidablesEnum.Suggestion: pGuild.settings.config.suggestionConfig = new SuggestionConfig({
                firstTimeExecuted, channel, managerRole, suggestions: []
            }); break;
            case DecidablesEnum.Theme: pGuild.settings.config.themeConfig = new ThemeConfig({
        ...
        return decidablesType == DecidablesEnum.Giveaway ? pGuild.settings.config.giveawayConfig :
            decidablesType == DecidablesEnum.Poll ? pGuild.settings.config.pollConfig : 
                decidablesType == DecidablesEnum.Suggestion ? pGuild.settings.config.suggestionConfig :
                    pGuild.settings.config.themeConfig;
...
async function AfterTimeOut(sent: Message, value: string, amountOfWinners: number, embed: MessageEmbed, decidable: Decidable, interval: NodeJS.Timeout, params: DecidablesParams, previousWinner: GuildMember) {
    ...
    async function GetGiveawayWinner(decidable: Giveaway | Theme) {
        ...
        const config = decidablesType == DecidablesEnum.Giveaway ? pGuild.settings.config.giveawayConfig : pGuild.settings.config.themeConfig,
...
async function RemoveDecidables(message: Message, pGuild: PinguGuild, type: DecidablesTypes, decidables: Decidable[]) {
    ...
    decidables.forEach(d => 
        type == DecidablesEnum.Giveaway ? pGuild.settings.config.giveawayConfig.giveaways.splice(pGuild.settings.config.giveawayConfig.giveaways.indexOf(d as Giveaway), 1) :
        type == DecidablesEnum.Poll ? pGuild.settings.config.pollConfig.polls.splice(pGuild.settings.config.pollConfig.polls.indexOf(d as Poll), 1) :
        type == DecidablesEnum.Suggestion ? pGuild.settings.config.suggestionConfig.suggestions.splice(pGuild.settings.config.suggestionConfig.suggestions.indexOf(d as Suggestion), 1) :
        pGuild.settings.config.themeConfig.themes.splice(pGuild.settings.config.themeConfig.themes.indexOf(d as Theme), 1));
    ...
    switch (type) {
        case DecidablesEnum.Giveaway: return pGuild.settings.config.giveawayConfig.giveaways as Decidable[];
        case DecidablesEnum.Poll: return pGuild.settings.config.pollConfig.polls as Decidable[];
        case DecidablesEnum.Suggestion: return pGuild.settings.config.suggestionConfig.suggestions as Decidable[];
        case DecidablesEnum.Theme: return pGuild.settings.config.themeConfig.themes as Decidable[];
    }
}
...
async function UpdatePGuild(client: Client, pGuild: PinguGuild, decidableType: DecidablesTypes, succMsg: string, errMsg: string) {
    return await PinguGuild.UpdatePGuild(client, 
        (decidableType == DecidablesEnum.Giveaway ? { giveawayConfig: pGuild.settings.config.giveawayConfig } :
            decidableType == DecidablesEnum.Poll ? { pollConfig: pGuild.settings.config.pollConfig } :
                decidableType == DecidablesEnum.Suggestion ? { suggestionConfig: pGuild.settings.config.suggestionConfig } :
                    { themeConfig: pGuild.settings.config.themeConfig }),
...
async function SaveVerdictToPGuilds(params: DecidablesParams, decidable: Decidable) {
    ...
    const arr = (
        decidablesType == DecidablesEnum.Giveaway ? pGuild.settings.config.giveawayConfig.giveaways : 
        decidablesType == DecidablesEnum.Poll ? pGuild.settings.config.pollConfig.polls : 
        decidablesType == DecidablesEnum.Suggestion ? pGuild.settings.config.suggestionConfig.suggestions : 
        pGuild.settings.config.themeConfig.themes) as Decidable[];
...
async function AddDecidableToPGuilds(params: DecidablesParams, decidable: Decidable) {
    ...
    switch (decidablesType) {
        case DecidablesEnum.Giveaway: pGuild.settings.config.giveawayConfig.giveaways.push(decidable as Giveaway);
        case DecidablesEnum.Poll: pGuild.settings.config.pollConfig.polls.push(decidable as Poll);
        case DecidablesEnum.Suggestion: pGuild.settings.config.suggestionConfig.suggestions.push(decidable as Suggestion);
        case DecidablesEnum.Theme: pGuild.settings.config.themeConfig.themes.push(decidable as Theme);
    }
