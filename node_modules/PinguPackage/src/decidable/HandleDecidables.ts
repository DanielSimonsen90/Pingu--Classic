...
import { GiveawayConfig, PollConfig, SuggestionConfig } from '../decidable/config'; //Decidable configs
import { Giveaway, Poll, Suggestion, Decidable } from '../decidable/items'; //Decidable items
...
interface DecidablesParams {
    ...
    listEmojis: string[],
    config: GiveawayConfig | PollConfig | SuggestionConfig
}
...
async function HandleDecidables(params: DecidablesParams) {
    ...
    if (reroll()) await message.channel.send(`Rerolling giveaway...`);
    ...
    //Giveaway reroll
    if (reroll()) return await Reroll(params, embed, pGuild.giveawayConfig.giveaways.find(g => g._id == value));
    ...
    let pRole =
        DecidablesTypes.Giveaway == decidablesType ? (config as GiveawayConfig).hostRole :
            DecidablesTypes.Poll == decidablesType ? (config as PollConfig).pollRole :
                (config as SuggestionConfig).managerRole
...
async function FirstTimeExecuted(params: DecidablesParams) {
    ...
    collector.on('collect', async input => {
        ...
        switch (collectorCount) {
            case 0: reply = Find('Role', hostDone ? 'Giveaway Winner' : staffRoleType, 'Create', userInput); break;
            case 1:
                if (!hostDone) {
                ...
                break;
            case 2: reply = Find('Role', hostDone ? 'Giveaway Winner' : staffRoleType, 'Create', userInput); break;
            case 3:
                ...
                if (decidablesType != DecidablesTypes.Giveaway) return GoodToGo({ staffRole, decidablesChannel });
    ...
    async function HasAllArguments() {
        ...
        if (decidablesType == DecidablesTypes.Giveaway && args.length != 5) return false; //Not enough arguments
        ...
        if (decidablesType == DecidablesTypes.Giveaway)
            await GoodToGo({ staffRole, decidablesChannel: channel, winnerRole: findRole(args[2]), allowSameWinner: ['yes', 'true'].includes(args[4].toLowerCase()) });
    ...
    async function GoodToGo(params: { staffRole: RoleResolvable, decidablesChannel: GuildChannelResolvable, winnerRole?: RoleResolvable, allowSameWinner?: boolean }) {
...
async function ListDecidables(params: DecidablesParams, decidables: Decidable[]) {
    ...
    function CreateEmbeds(autocalled: boolean) {
        ...
        for (var i = 0; i < decidables.length; i++) {
            try {
                embeds[i] = (decidablesType == DecidablesTypes.Giveaway ? CreateGiveawayEmbed(decidables[i] as Giveaway) :
...
async function AfterTimeOut(sent: Message, value: string, amountOfWinners: number, embed: MessageEmbed, decidable: Decidable, interval: NodeJS.Timeout, params: DecidablesParams, previousWinner: GuildMember) {
    ...
    else GetGiveawayWinner(decidable as Giveaway);
    ...
    async function GetGiveawayWinner(giveaway: Giveaway) {
        const giveawayCreator = sent.guild.member(decidable.author._id);
        const pGuildGiveaway = pGuild.giveawayConfig,
        ...
        try { var peopleReacted = (await sent.reactions.cache.get('🤞').users.fetch()).array(); }
        catch (err) {
            await PinguLibrary.errorLog(sent.client, `Fetching 🤞 reactions from giveaway`, sent.content, err);
        ...
        for (var i = 0; i < winners.length; i++) {
            await sent.guild.member(winners[i]).roles.add(GiveawayWinnerRole._id)
                .catch(async err => {
                    if (err != `TypeError [INVALID_TYPE]: Supplied roles is not a Role, Snowflake or Array or Collection of Roles or Snowflakes.`) {
                        await PinguLibrary.errorLog(sent.client, `Unable to give <@${(winner as User).id}> "${sent.guild.name}"'s Giveaway Winner Role, ${GiveawayWinnerRole.name} (${GiveawayWinnerRole._id})`, message.content, err);
                        giveawayCreator.user.send(`I couldn't give <@${(winner as User).id}> a Giveaway Winner role! I have already notified my developers.`);
                    }
        ...
        sent.edit(embed
            ...
            .setFooter('Giveaway ended.')
        ).catch(err => PinguLibrary.errorLog(sent.client, `Editing the Giveaway Message`, sent.content, err)
        ...
        async function UpdatePGuildWinners() {
            for (var i = 0; i < winners.length; i++) {
                giveaway.winners.push(new PGuildMember(sent.guild.member(winners[i])));
            }
        }
    }
}
async function RemoveDecidables(message: Message, pGuild: PinguGuild, type: DecidablesTypes, decidables: Decidable[]) {
    ...
    decidables.forEach(d => type == DecidablesTypes.Giveaway ? pGuild.giveawayConfig.giveaways.splice(pGuild.giveawayConfig.giveaways.indexOf(d as Giveaway), 1) :
        type == DecidablesTypes.Poll ? pGuild.pollConfig.polls.splice(pGuild.pollConfig.polls.indexOf(d as Poll), 1) :
            pGuild.suggestionConfig.suggestions.splice(pGuild.suggestionConfig.suggestions.indexOf(d as Suggestion), 1));
...
async function Reroll(params: DecidablesParams, embed: MessageEmbed, giveaway: Giveaway) {
    let { message, args } = params;
    if (!args[1]) return message.edit(`Giveaway message not found - please provide with a message ID`);
    ...
    return await AfterTimeOut(
        ...
        null,
        params,
        message.guild.members.cache.get(giveaway.winners[0]._id)
    );
}
...
async function SaveVerdictToPGuilds(params: DecidablesParams, decidable: Decidable) {
    ...
    const arr = (decidablesType == DecidablesTypes.Giveaway ? pGuild.giveawayConfig.giveaways : decidablesType == DecidablesTypes.Poll ? pGuild.pollConfig.polls : pGuild.suggestionConfig.suggestions) as Decidable[];
