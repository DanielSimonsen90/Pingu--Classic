...
import { Guild, GuildChannel, GuildMember, GuildEmoji, Role } from 'discord.js' //Guild stuff
import { Message, MessageEmbed, TextChannel, MessageReaction, User, Client } from 'discord.js'; //Discord classes

import { PClient, PGuildMember, PRole, PChannel } from '../database/index'; //PItems
...
async function HandleDecidables(params: DecidablesParams) {
    ...
    const reroll = () => (Is(DecidablesTypes.Giveaway) || Is(DecidablesTypes.Theme)) && args[0] == `reroll`;
    ...
    let permCheck = await PermissionCheck(params);
    ...
    if (firstTimeExecuted || args[0] == 'setup')
        ...
        Is(DecidablesTypes.Giveaway) ? pGuild.giveawayConfig.giveaways :
            Is(DecidablesTypes.Poll) ? pGuild.pollConfig.polls :
            Is(DecidablesTypes.Suggestion) ? pGuild.suggestionConfig.suggestions :
            pGuild.themeConfig.themes
    );
    ...
    if (!Is(DecidablesTypes.Suggestion)) {
        ...
        else if (Is(DecidablesTypes.Giveaway) || Is(DecidablesTypes.Theme)) {
            var winners = 1;
    ...
    let embed = new MessageEmbed()
        ...
        .setDescription((
            Is(DecidablesTypes.Giveaway) || Is(DecidablesTypes.Theme) ? (
    ...
    //Giveaway reroll
    if (reroll()) return await Reroll(params, embed, (Is(DecidablesTypes.Giveaway) ? pGuild.giveawayConfig.giveaways : pGuild.themeConfig.themes).find(d => d._id == value));
    ...
    if (Is(DecidablesTypes.Suggestion)) return;
    ...
    function Is(type: DecidablesTypes) {
        return decidablesType == type;
    }
}

async function PermissionCheck(params: DecidablesParams) {
    let { message, config, args, decidablesType, pGuild } = params;
    let pRole =
        DecidablesTypes.Giveaway == decidablesType ? (config as GiveawayConfig).hostRole :
            DecidablesTypes.Poll == decidablesType ? (config as PollConfig).pollRole : 
            DecidablesTypes.Suggestion == decidablesType ? (config as SuggestionConfig).managerRole : 
    ...
    if (decidablesType == DecidablesTypes.Suggestion) return PinguLibrary.PermissionGranted;
    ...
    async function CheckRoleUpdates() {
        ...
        let winnerPRole =  DecidablesTypes.Giveaway ? pGuild.giveawayConfig.winnerRole : 
                           DecidablesTypes.Theme ? pGuild.themeConfig.winnerRole : undefined;
        ...
        if (!winnerRole && winnerPRole != undefined || winnerPRole != undefined && winnerRole || //Winner role (doesn't) exist(s)
            ...
            await UpdatePGuild(message.client, pGuild, decidablesType,
                `Updated ${decidablesType.toLowerCase()} role${decidablesType == DecidablesTypes.Giveaway ? 's' : ''}`,
                `I encountered and error while updating ${decidablesType.toLowerCase()} role${decidablesType == DecidablesTypes.Giveaway ? 's' : ''}`
            );
...
async function FirstTimeExecuted(params: DecidablesParams) {
    ...
    let collector = message.channel.createMessageCollector(m => (m as Message).author.id == message.author.id, { max: 1 }),
    ...
    collector.on('collect', async input => {
        ...
        switch (collectorCount) {
            case 0: reply = Find('Role', hostDone ? `${decidablesType} Winner` : staffRoleType, 'Create', userInput); break;
            case 1:
                if (!hostDone) {
                    ...
                    if ([DecidablesTypes.Giveaway, DecidablesTypes.Theme].includes(decidablesType)) {
                ...
                break;
            case 2: reply = Find('Role', hostDone ? `${decidablesType} Winner` : staffRoleType, 'Create', userInput); break;
            case 3:
                ...
                if ([DecidablesTypes.Giveaway, DecidablesTypes.Theme]) return GoodToGo({ staffRole, decidablesChannel });
                ...
                break;
            case 4: 
                var allowSameWinner = lastInput == 'yes';
                if (DecidablesTypes.Giveaway == decidablesType) return GoodToGo({ staffRole, decidablesChannel, winnerRole, allowSameWinner });
                reply = `Okay seriously last thing, how many previous themes' winners should I ignore? (If set to 2, the winners for the last 2 themes will not be able to win the current one) Default is 0`
                break;
    ...
    async function HasAllArguments() {
        ...
        if (DecidablesTypes.Giveaway == decidablesType && args.length != 5) return false; //Not enough arguments
        else if (DecidablesTypes.Theme == decidablesType && args.length != 6) return false //Not enough arguments
        ...
        if ([DecidablesTypes.Giveaway, DecidablesTypes.Theme].includes(decidablesType))
            await GoodToGo({ 
                ...
                ignoreLastWins: DecidablesTypes.Theme == decidablesType && !isNaN(parseInt(args[5])) ? parseInt(args[5]) : 0 });
     ...
     */
    function Find(type: 'Role' | 'Channel', typeName: string, pinguResponse: 'Exists' | 'Create' | 'Find', userInput: Message): string {
    ...
    async function GoodToGo(params: { staffRole: RoleResolvable, decidablesChannel: GuildChannelResolvable, winnerRole?: RoleResolvable, allowSameWinner?: boolean, ignoreLastWins?: number }) {
        ...
        switch (decidablesType) {
            case DecidablesTypes.Giveaway: pGuild.giveawayConfig = new GiveawayConfig({
                firstTimeExecuted: false,
                allowSameWinner,
                channel: new PChannel(decidablesChannel as GuildChannel),
                hostRole: new PRole(staffRole as Role),
                winnerRole: new PRole(winnerRole as Role),
                giveaways: []
            }); break;
            case DecidablesTypes.Poll: pGuild.pollConfig = new PollConfig({
                firstTimeExecuted: false,
                channel: new PChannel(decidablesChannel as GuildChannel),
                pollRole: new PRole(staffRole as Role),
                polls: []
            }); break;
            case DecidablesTypes.Suggestion: pGuild.suggestionConfig = new SuggestionConfig({
                firstTimeExecuted: false,
                channel: new PChannel(decidablesChannel as GuildChannel),
                managerRole: new PRole(staffRole as Role),
                suggestions: []
            }); break;
            case DecidablesTypes.Theme: pGuild.themeConfig = new ThemeConfig({
                firstTimeExecuted: false,
                allowSameWinner,
                ignoreLastWins,
                channel: new PChannel(decidablesChannel as GuildChannel),
...
async function ListDecidables(params: DecidablesParams, decidables: Decidable[]) {
    ...
    if (decidablesType == DecidablesTypes.Suggestion) listEmojis[1] = 'Checkmark';
    ...
    reactionCollector.on('collect', async reaction => {
        let decidable = decidables.find(d => d._id == (
            [DecidablesTypes.Giveaway, DecidablesTypes.Theme].includes(decidablesType) ?
    ...
    function CreateEmbeds(autocalled: boolean) {
        ...
        for (var i = 0; i < decidables.length; i++) {
            try {
                embeds[i] = ([DecidablesTypes.Giveaway, DecidablesTypes.Theme].includes(decidablesType) ? CreateGiveawayEmbed(decidables[i] as Giveaway) :
                    decidablesType == DecidablesTypes.Poll ? CreatePollEmbed(decidables[i] as Poll) : CreateSuggestionEmbed(decidables[i] as Suggestion))
                    .setColor(pGuildClient.embedColor)
...
async function AfterTimeOut(sent: Message, value: string, amountOfWinners: number, embed: MessageEmbed, decidable: Decidable, interval: NodeJS.Timeout, params: DecidablesParams, previousWinner: GuildMember) {
    ...
    if (decidablesType == DecidablesTypes.Poll) DecidePoll(decidable as Poll);
    else GetGiveawayWinner(decidablesType == DecidablesTypes.Giveaway ? decidable as Giveaway : decidable as Theme);
    ...
    async function GetGiveawayWinner(decidable: Giveaway | Theme) {
        ...
        const config = decidablesType == DecidablesTypes.Giveaway ? pGuild.giveawayConfig : pGuild.themeConfig,
        ...
        peopleReacted = peopleReacted.filter(user =>
            ...
            allowSameWinner != null && ( //allowSameWinner exists
                ...
                (decidablesType == DecidablesTypes.Theme && ( //decidableType is theme & not giveaway
...
async function RemoveDecidables(message: Message, pGuild: PinguGuild, type: DecidablesTypes, decidables: Decidable[]) {
    ...
    decidables.forEach(d => 
        type == DecidablesTypes.Giveaway ? pGuild.giveawayConfig.giveaways.splice(pGuild.giveawayConfig.giveaways.indexOf(d as Giveaway), 1) :
        type == DecidablesTypes.Poll ? pGuild.pollConfig.polls.splice(pGuild.pollConfig.polls.indexOf(d as Poll), 1) :
        type == DecidablesTypes.Suggestion ? pGuild.suggestionConfig.suggestions.splice(pGuild.suggestionConfig.suggestions.indexOf(d as Suggestion), 1) :
        pGuild.themeConfig.themes.splice(pGuild.themeConfig.themes.indexOf(d as Theme), 1));
    ...
    switch (type) {
        case DecidablesTypes.Giveaway: return pGuild.giveawayConfig.giveaways as Decidable[];
        case DecidablesTypes.Poll: return pGuild.pollConfig.polls as Decidable[];
        case DecidablesTypes.Suggestion: return pGuild.suggestionConfig.suggestions as Decidable[];
        case DecidablesTypes.Theme: return pGuild.themeConfig.themes as Decidable[];
    }
}
...
async function UpdatePGuild(client: Client, pGuild: PinguGuild, decidableType: DecidablesTypes, succMsg: string, errMsg: string) {
    return await PinguGuild.UpdatePGuild(client,
        decidableType == DecidablesTypes.Giveaway ? { giveawayConfig: pGuild.giveawayConfig } :
            decidableType == DecidablesTypes.Poll ? { pollConfig: pGuild.pollConfig } :
            decidableType == DecidablesTypes.Suggestion ? { suggestionConfig: pGuild.suggestionConfig } :
            { themeConfig: pGuild.themeConfig },
...
async function SaveVerdictToPGuilds(params: DecidablesParams, decidable: Decidable) {
    ...
    const arr = (
        decidablesType == DecidablesTypes.Giveaway ? pGuild.giveawayConfig.giveaways : 
        decidablesType == DecidablesTypes.Poll ? pGuild.pollConfig.polls : 
        decidablesType == DecidablesTypes.Suggestion ? pGuild.suggestionConfig.suggestions : 
        pGuild.themeConfig.themes) as Decidable[];
...
async function AddDecidableToPGuilds(params: DecidablesParams, decidable: Decidable) {
    ...
    switch (decidablesType) {
        case DecidablesTypes.Giveaway: pGuild.giveawayConfig.giveaways.push(decidable as Giveaway);
        case DecidablesTypes.Poll: pGuild.pollConfig.polls.push(decidable as Poll);
        case DecidablesTypes.Suggestion: pGuild.suggestionConfig.suggestions.push(decidable as Suggestion);
        case DecidablesTypes.Theme: pGuild.themeConfig.themes.push(decidable as Theme);
    }
...
export { HandleDecidables, DecidablesTypes };