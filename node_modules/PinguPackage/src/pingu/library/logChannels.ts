...
import { PinguEvents } from "../events";
import { config } from '../..';
const { version } = config;


export async function errorLog(client: Client, message: string, messageContent?: string, err?: Error) {
    var errorlogChannel = PinguLibrary.getChannel(client, PinguLibrary.SavedServers.PinguSupport(client).id, 'error-log-⚠');
    if (!errorlogChannel) return PinguLibrary.DanhoDM(client, 'Unable to find #error-log-⚠️ in Pingu Support, https://discord.gg/gbxRV4Ekvh');
...
export async function pGuildLog(client: Client, script: string, message: string, err ?: Error) {
    var pinguGuildLog = PinguLibrary.getChannel(client, this.SavedServers.PinguSupport(client).id, "pingu-guild-log-🏡");
    if (!pinguGuildLog) return PinguLibrary.DanhoDM(client, `Couldn't get #pingu-guild-log-🏡 in Pingu Support, https://discord.gg/gbxRV4Ekvh`)

    if (err) {
        var errorLink = (await PinguLibrary.errorLog(client, `pGuild Error: "${message}"`, null, err)).url;
...
export async function pUserLog(client: Client, script: string, message: string, err ?: Error) {
    var pinguUserLog = PinguLibrary.getChannel(client, this.SavedServers.PinguSupport(client).id, "pingu-user-log-🧍");
    if (!pinguUserLog) return PinguLibrary.DanhoDM(client, `Couldn't get #pingu-user-log-🧍 in Pingu Support, https://discord.gg/gbxRV4Ekvh`)

    if (err) {
        var errorLink = (await PinguLibrary.errorLog(client, `pUser Error (**${script}**): "${message}"`, null, err)).url;
...
export async function consoleLog(client: Client, message: string) {
    ...
    let consoleLogChannel = PinguLibrary.getChannel(client, this.SavedServers.PinguSupport(client).id, "console-log-📝");
    if (!consoleLogChannel) return PinguLibrary.DanhoDM(client, 'Unable to find #console-log-📝 in Pingu Support, https://discord.gg/gbxRV4Ekvh');
...
export async function eventLog(client: Client, content: MessageEmbed) {
    ...
    let eventLogChannel = PinguLibrary.getChannel(client, this.SavedServers.PinguSupport(client).id, "event-log-📹");
    if (!eventLogChannel) return PinguLibrary.DanhoDM(client, `Couldn't get #event-log-📹 channel in Pingu Support, https://discord.gg/gbxRV4Ekvh`)
...
export async function tellLog(client: Client, sender: User, reciever: User, message: Message | MessageEmbed) {
    ...
    var tellLogChannel = PinguLibrary.getChannel(client, this.SavedServers.PinguSupport(client).id, 'tell-log-💬');
    if (!tellLogChannel) return PinguLibrary.DanhoDM(client, `Couldn't get #tell-log-💬 channel in Pingu Support, https://discord.gg/gbxRV4Ekvh`)
...
export async function latencyCheck(message: Message) {
    ...
    if (!pingChannel) return PinguLibrary.DanhoDM(message.client, `Couldn't get #ping-log-🏓 channel in Pingu Support, https://discord.gg/gbxRV4Ekvh`)
