...
export async function errorLog(client: Client, message: string, messageContent?: string, err?: Error, errorID?: number) {
    ...
    if (!PinguLibrary.errorCache.get(errorID) && err) fs.writeFile(errorFilePath, JSON.stringify(err), () => PinguLibrary.consoleLog(client, `Created error file for error #${errorID}.`));
