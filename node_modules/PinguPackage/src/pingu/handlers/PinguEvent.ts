...
export async function HandleEvent<EventType extends keyof PinguClientEvents>(caller: EventType, client: Client, path: string, ...args: PinguClientEvents[EventType]) {
    ...
    try {
        try { if (event.execute != null) event.execute(ToPinguClient(client), ...args); } catch (err) { PinguLibrary.errorLog(client, `${event.name}.execute`, null, new Error(err)); throw `execute`; }
        try { if (event.setContent != null) await SendToLog(); } catch (err) { PinguLibrary.errorLog(client, `${event.name}.setContent`, null, new Error(err)); throw `setContent`; }
    }
    catch (cause) {
    ...
    async function SendToLog() {
        ...
        if (embed) return await PinguLibrary.eventLog(ToPinguClient(client), embed);

        async function GetFromAuditLog() {
            const noAuditLog = PinguEvent.noAuditLog;

            switch (event.name) {
                case 'channelCreate': return !(parameters.channel as GuildChannel).guild ? (parameters.channel as DMChannel).recipient.tag : await GetInfo((parameters.channel as GuildChannel).guild, 'CHANNEL_CREATE');
                case 'channelUpdate': return !(parameters.channel as GuildChannel).guild ? (parameters.channel as DMChannel).recipient.tag : await GetInfo((parameters.channel as GuildChannel).guild, 'CHANNEL_UPDATE');
                case 'channelDelete': return !(parameters.channel as GuildChannel).guild ? (parameters.channel as DMChannel).recipient.tag : await GetInfo((parameters.channel as GuildChannel).guild, 'CHANNEL_DELETE');
                case 'channelPinsUpdate': return !(parameters.channel as GuildChannel).guild ? (parameters.channel as DMChannel).recipient.tag :
                    (await GetInfo((parameters.channel as GuildChannel).guild, 'MESSAGE_PIN') ||
                        await GetInfo((parameters.channel as GuildChannel).guild, 'MESSAGE_UNPIN'));

                case 'webhookCreate': return await GetInfo((parameters.channel as GuildChannel).guild, 'WEBHOOK_CREATE');
                case 'webhookUpdate': return await GetInfo((parameters.channel as GuildChannel).guild, 'WEBHOOK_UPDATE');
                case 'webhookDelete': return await GetInfo((parameters.channel as GuildChannel).guild, 'WEBHOOK_DELETE');
