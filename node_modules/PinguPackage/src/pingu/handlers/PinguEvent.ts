...
export async function HandleEvent<EventType extends keyof PinguClientEvents>(caller: EventType, client: Client, path: string, ...args: PinguClientEvents[EventType]) {
    ...
    var parameters = args.reduce((result, arg) => Object.assign(result, arg), {} as never);
    ...
    var [user, guild, guildMember] = [
        ...
        parameters.last && parameters.last()) as User,
        (parameters.guild || FindClass("Guild")) as Guild,
        (parameters.member || FindClass("GuildMember")) as GuildMember,
    ];
    ...
    async function SendToLog() {
        let emitAssociator: string = 
            parameters.author && parameters.author.tag ||
            parameters.tag || 
        ...
        if (caller == 'message' && ['event-log-📹', 'ping-log-🏓', 'console-log-📝'].includes((parameters.channel as GuildChannel).name)) return;
        ...
        async function GetFromAuditLog() {
            ...
            switch (event.name) {
                case 'channelCreate': return !parameters.guild ? parameters.recipient.tag : GetInfo(parameters.guild, 'CHANNEL_CREATE');
                case 'channelUpdate': return !parameters.guild ? parameters.recipient.tag : GetInfo(parameters.guild, 'CHANNEL_UPDATE');
                case 'channelDelete': return !parameters.guild ? parameters.recipient.tag : GetInfo(parameters.guild, 'CHANNEL_DELETE');
                case 'channelPinsUpdate': return !parameters.guild ? parameters.recipient.tag :
                    (await GetInfo(parameters.guild, 'MESSAGE_PIN') || GetInfo(parameters.guild, 'MESSAGE_UNPIN'));

                case 'webhookCreate': return GetInfo(parameters.guild, 'WEBHOOK_CREATE');
                case 'webhookUpdate': return GetInfo(parameters.guild, 'WEBHOOK_UPDATE');
                case 'webhookDelete': return GetInfo(parameters.guild, 'WEBHOOK_DELETE');

                case 'emojiCreate': return GetInfo(parameters.guild, 'EMOJI_CREATE');
                case 'emojiUpdate': return GetInfo(parameters.guild, 'EMOJI_UPDATE');
                case 'emojiDelete': return GetInfo(parameters.guild, 'EMOJI_DELETE');

                case 'guildBanAdd': return GetInfo(parameters, 'MEMBER_BAN_ADD');
                case 'guildMemberUpdate': return GetInfo(parameters.guild, 'MEMBER_UPDATE');
                case 'guildBanRemove': return GetInfo(parameters, 'MEMBER_BAN_REMOVE');

                case 'guildUpdate': return GetInfo(parameters, 'GUILD_UPDATE');
                case 'guildIntegrationsUpdate': return GetInfo(parameters, 'INTEGRATION_UPDATE');

                case 'messageBulkDelete': return GetInfo(parameters.last().guild, 'MESSAGE_BULK_DELETE');

                case 'roleCreate': return GetInfo(parameters.guild, 'ROLE_CREATE');
                case 'roleUpdate': return GetInfo(parameters.guild, 'ROLE_UPDATE');
                case 'roleDelete': return GetInfo(parameters.guild, 'ROLE_DELETE');
        ...
        async function CreateEmbed() {
            ...
            async function getColor(): Promise<ColorResolvable> {
                ...
                try { return (await PinguGuild.Get(PinguLibrary.SavedServers.PinguSupport(client))).clients.find(c => c._id == client.user.id).embedColor; }
