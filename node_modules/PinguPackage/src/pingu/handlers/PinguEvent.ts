...
export async function HandleEvent<EventType extends keyof PinguClientEvents>(caller: EventType, _client: Client, path: string, ...args: PinguClientEvents[EventType]) {
    ...
    async function execute() {
        try { return event.execute(ToPinguClient(_client), ...args); } 
    ...
    async function SendToLog() {
        ...
        if (emitAssociator.match(/#\d{4}$/g)) {
            ...
            let user = _client.users.cache.find(u => u.tag == emitAssociator);
            if (user) { 
        ...
        async function CreateEmbed() {
            let [user, guild, executed] = [
                _client.users.cache.find(u => u.tag == emitAssociator),
                _client.guilds.cache.find(g => g.name == emitAssociator),
                new Date()
            ];
