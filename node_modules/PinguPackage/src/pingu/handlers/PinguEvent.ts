...
export async function HandleEvent<EventType extends keyof PinguClientEvents>(caller: EventType, client: Client, path: string, ...args: PinguClientEvents[EventType]) {
    ...
    var parameters = (function CombineEmbeds() {
        let result = {} as any;
        for (var arg in args) {
            let argKeys = Object.keys(args[arg]);
            for (var i in argKeys) {
                let prop = args[arg][argKeys[i]];
                if (prop && !result[argKeys[i]])
                    result[argKeys[i]] = prop;
            }
        }
        return result;
    })();
    ...
    async function SendToLog() {
        ...
        let emitAssociator = "";
        for (let i = 0; i < emitAssociatorOptions.length; i++) {
            if (emitAssociator) break;
            else emitAssociator = emitAssociatorOptions[i];
        }
