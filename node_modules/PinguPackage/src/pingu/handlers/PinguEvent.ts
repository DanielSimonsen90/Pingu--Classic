...
export async function HandleEvent<EventType extends keyof PinguClientEvents>(caller: EventType, client: Client, path: string, ...args: PinguClientEvents[EventType]) {
    ...
    try {
        try { if (event.execute != null) await event.execute(ToPinguClient(client), ...args); } catch (err) { PinguLibrary.errorLog(client, `${event.name}.execute`, null, new Error(err)); throw `execute`; }
