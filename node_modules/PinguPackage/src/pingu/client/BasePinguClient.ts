...
import PinguCollection from '../../helpers/PinguCollection';
import DeveloperCollection, { developers } from '../../helpers/DeveloperCollection'
...
export abstract class BasePinguClient<Events extends ClientEvents = any> extends Client {
    ...
    public async DBExecute<T>(callback: (mongoose: typeof import('mongoose')) => Promise<T>) { return DBExecute(this, callback); }
    ...
    protected async onceReady() {
        ...
        this.logChannels = (() => { 
            return new Collection<LogChannels, TextChannel>(
                new Collection<LogChannels, string>([
                    ...
                    const logChannels = this.savedServers.get('Pingu Support').channels.cache.find(c => c.name.includes('Log Channels')) as CategoryChannel;
        ...
        await Promise.all([ this.pGuilds.refresh(this), this.pUsers.refresh(this), this.emotes.refresh(this) ]);
    }
    ...
    private async consoleLog(channel: TextChannel, message: string, type: ConsoleLogType = 'log') {
        new Map<ConsoleLogType, (...data: any[]) => void>([
            ['log', console.log],
            ['warn', console.warn],
            ['error', console.error]
        ]).get(type)(`[${new Date().toLocaleTimeString()}] ${message}`);
    ...
    private async errorLog(channel: TextChannel, message: string, messageContent?: string, err?: Error, params?: ErrorLogParams): Promise<Message> {
        ...
        let sent = await sendMessage(this, getErrorMessage(message, messageContent, err)).catch(err => sentCatcher(err, 'txt'));
        let paramsSent = await sendMessage(this, "```\n[Parameters]:\n" + jsonErrorContent + "\n```").catch(err => sentCatcher(err, 'json'));
        ...
        async function sendMessage(client: BasePinguClient<any>, content: string) {
            console.error(content.includes('`') ? content.replace('`', ' ') : content);

            let sent = await channel.send(content);
            await sent.react(client.savedServers.get('Danho Misc').emojis.cache.find(e => e.name == 'Checkmark')); //Mark as solved
            await sent.react('📄'); //Get error file
            
            //Create reaction handler
            sent.createReactionCollector(() => true).on('collect', async (reaction, user) => {
                if (!client.developers.isPinguDev(user) || !reaction.users.cache.has(client.user.id)) return reaction.remove();

                if (reaction.emoji.name == '📄') {
                    let fileMessage = await reaction.message.channel.send(new MessageAttachment(`${errorPath}/${errorID}.json`, `Error ${errorID}.json`));
                    return client.cache.errors.set(errorID, [...client.cache.errors.get(errorID), fileMessage]);
                }

                client.cache.errors.get(errorID).forEach(m => m.delete({ reason: `Error #${errorID}, was marked as solved by ${user.tag}` }));
                const errorFiles = fs.readdirSync(errorPath).filter(file => file.startsWith(errorID.toString()));
                errorFiles.forEach(file => fs.unlink(`${errorPath}/${file}`, () => client.log('console', `Deleted error #${errorID}.`)))
            })
    ...
    private async raspberryLog(channel: TextChannel) {
        if (!this.isLive) return null;
        return channel.send(`Booted on version **${this.config.version}**.`);
    }
