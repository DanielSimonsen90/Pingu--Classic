...
>  {
    ...
    private get _embed() { return this._embeds[this._embedIndex]; }
    ...
    private async _onVerdict(approved: boolean, by: GuildMember) {
        if (!this._data.is('Suggestion')) return this._embed;

        this._collection = await this._decide(approved, this._getEmbedDecdiable(this._embed) as Suggestion, by);
    ...
    private async _direction(direction: number): Promise<MessageEmbed> {
        ...
        return direction == 0 ? this._deleteDecidable(this._embed) : this._embed;
    }
