//#region Variables
const { Command, Client, Collection, Guild, Message, MessageEmbed, User } = require('discord.js'),
    { token, Prefix, announceOutages } = require('./config.json'),
    { musicCommands } = require('./commands/2 Fun/music'),
    { GetMention } = require('./commands/2 Fun/tell'),
    { CategoryNames } = require('./commands/4 DevOnly/update'),
    { PinguGuild, PinguLibrary, PinguUser, DiscordPermissions, Error, PUser } = require('./PinguPackage'),
    fs = require('fs'),
    ...
    function UpdateSharedServers() {
        ...
        if (pUser.sharedServers.length == 0)
            PinguUser.DeletePUser(member.user, async pUser =>
                PinguLibrary.pUserLog(client, "index: guildMemberRemove", `Successfully deleted ${pUser.tag} from Pingu Users.`)
            );
        else PinguUser.UpdatePUsersJSON(client, member.user, "index: guildMemberRemove", `Successfully removed **${member.guild.name}** from **${pUser.tag}**'s SharedServers.`)
    }
...
async function ExecuteTellReply(message) {
    ...
    if (!replyPersonPUser) return console.log(`No replyPerson found for ${message.author.username}.`);
    ...
    if (replyPersonPinguUser.replyPerson.id != message.author.id) {
        ...
        PinguUser.UpdatePUsersJSONAsync(client, replyPerson, "index: ExecuteTellReply",
    ...
    //Log conversation
    try { PinguLibrary.tellLog(client, message.author, replyPerson, message); }
...
function getReplyUser(message) {
    ...
    function getGuildUsers() {
        let guildUsersArr = client.guilds.cache.map(guild => guild.members.cache.map(gm => !gm.user.bot && gm.user));
